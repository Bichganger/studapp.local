...
async function load() {
    ...
    if(data.length===0) { tb.innerHTML='<tr><td colspan="5" class="text-center">Нет работ</td></tr>'; return; }
    tb.innerHTML = data.map(x=>`
        <tr><td>${x.subject}</td><td>${x.title}</td>
        <td><span class="badge bg-${x.status==='accepted'?'success':x.status==='rejected'?'danger':'warning'}">${x.status||'new'}</span></td>
        <td>${x.grade||'-'}</td><td>${x.teacher_comment||'-'}</td></tr>
    `).join('');
}
async function submitWork() {
    const fd = new FormData();
    fd.append('action','add_assignment');
    fd.append('subject',document.getElementById('aSubject').value);
